...
namespace chess
{
    ...
    int lsb(Bitboard bb)
    {
        return std::numeric_limits<Bitboard>::digits - std::__countl_zero(bb & -bb) - 1;
    }
    ...
    std::vector<Square> scan_forward(Bitboard bb)
    {
        ...
        {
            Bitboard r = bb & -bb;
            iter.push_back(std::numeric_limits<Bitboard>::digits - std::__countl_zero(r) - 1);
            bb ^= r;
        }
        return iter;
    }
    ...
    int msb(Bitboard bb)
    {
        return std::numeric_limits<Bitboard>::digits - std::__countl_zero(bb) - 1;
    }
    ...
    std::vector<Square> scan_reversed(Bitboard bb)
    {
        ...
        while (bb)
        {
            Square r = std::numeric_limits<Bitboard>::digits - std::__countl_zero(bb) - 1;
            iter.push_back(r);
            bb ^= BB_SQUARES[r];
        }
        return iter;
